...
import com.foodordering.order.config.RabbitMQConfig;
import com.foodordering.order.event.OrderCreatedEvent;
...
public class OrderController {
    ...
    @Autowired
    public OrderController(OrderRepository orderRepository, JwtUtil jwtUtil, RabbitTemplate rabbitTemplate) {
    ...
    /**
     * Fallback method to check admin privileges using JWT token (for backwards
     * compatibility)
     */
    ...
    public ResponseEntity<?> createOrder(@RequestBody Order order) {
        ...
        // FIXED: Properly establish bidirectional relationship between Order and
        // OrderItems
        ...
        OrderCreatedEvent event = new OrderCreatedEvent(
                ...
                "CREDIT_CARD" // Default for now, or get from request
        );
    ...
    // Get order by ID - User can only access their own orders, admins can access
    // all
    @GetMapping("/{id}")
    ...
    // Get orders by user - Users can only access their own orders, admins can
